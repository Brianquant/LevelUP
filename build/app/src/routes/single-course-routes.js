...
router.get('/single-course', async (req, res) => {
  ...
  var [courseData] = await connection.query('SELECT k.kurs_id, k.bezeichnung, k.anz_der_klausuren, exp FROM kurs AS k INNER JOIN klasse_kurs AS kk ON k.kurs_id = kk.kurs_id INNER JOIN klasse AS kl ON kl.klasse_id = kk.klasse_id INNER JOIN benutzer_kurs AS bk ON k.kurs_id = bk.kurs_id WHERE bk.user_id = ?;', [userData.user_id]);
